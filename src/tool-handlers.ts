...
export const tools: Tool[] = [
  ...
  {
    name: "connect_wifi",
    description: "Connect to an Android device over WiFi ADB. The device must have wireless debugging enabled.",
    inputSchema: {
      type: "object",
      properties: {
        ip: {
          type: "string",
          description: "Device IP address (e.g., 192.168.0.50)",
        },
        port: {
          type: "number",
          description: "ADB port (usually shown in wireless debugging settings, e.g., 44701)",
        },
      },
      required: ["ip", "port"],
    },
  ...
  {
    name: "pair_wifi",
    description: "Pair with an Android device over WiFi ADB (Android 11+). Use the pairing code and port from the device's wireless debugging settings.",
    inputSchema: {
      type: "object",
      properties: {
        ip: {
        ...
        port: {
          type: "number",
          description: "Pairing port (different from connection port)",
        },
        code: {
          type: "string",
          description: "6-digit pairing code shown on the device",
        },
      },
      required: ["ip", "port", "code"],
    },
  },
  {
    name: "disconnect_wifi",
    description: "Disconnect from a WiFi ADB device. If no ip/port given, disconnects all WiFi devices.",
    inputSchema: {
      type: "object",
      properties: {
        ip: {
          type: "string",
          description: "Device IP address (optional - disconnects all if omitted)",
        },
        port: {
          type: "number",
          description: "ADB port (optional)",
        },
      },
    },
  },
...
// Screenshot hash cache - avoids sending duplicate images
let lastScreenshotHash: string = '';

function computeScreenHash(data: string): string {
...
export async function handleTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case "screenshot": {
      ...
      // Hash-based deduplication: skip sending if screen unchanged
      const hash = computeScreenHash(result.data);
      if (!force && hash === lastScreenshotHash) {
        return { text: `Screen unchanged (hash:${hash.slice(0, 8)}). Use force=true to get image.` };
      }
    ...
    case "connect_wifi": {
      ...
      if (!ip || !port) {
        return { text: "Missing required parameters: ip and port" };
      }
      const result = deviceManager.getAndroidClient().connectWifi(ip, port);
      return { text: result };
    }
    ...
    case "pair_wifi": {
      ...
      if (!ip || !port || !code) {
        return { text: "Missing required parameters: ip, port, and code" };
      }
      const result = deviceManager.getAndroidClient().pairWifi(ip, port, code);
      return { text: result };
    }
